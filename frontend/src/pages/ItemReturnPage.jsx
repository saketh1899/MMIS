...
export default function ReturnItemPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* BLUE HEADER */}
      <div className="w-full bg-blue-600 text-white text-center py-2 mb-2 shadow-md">
        <h1 className="text-xl font-bold">Return Item</h1>
      </div>

      <div className="max-w-3xl mx-auto px-4">
        {/* ITEM CARD */}
        <div className="bg-white border rounded-lg shadow p-3 mb-2">
          <div className="flex gap-4">
              ...
              {getImageUrl(tx.item_image_url) ? (
                <div className="relative group">
                  <img 
                    ...
                    className="w-28 h-28 object-contain rounded-lg border border-gray-300 shadow-sm bg-white p-1 cursor-pointer hover:shadow-md transition-all duration-200"
                  ...
                  <p className="text-[10px] text-gray-500 text-center">Click to enlarge</p>
                </div>
              ) : null}
              <div 
                className="w-28 h-28 bg-gray-200 rounded-lg border border-gray-300 flex items-center justify-center text-gray-400 text-xs shadow-sm"
            ...
            <div className="flex-1 min-w-0">
              <h2 className="text-lg font-bold text-gray-800 truncate">{tx.item_name || `Item #${tx.item_id}`}</h2>
              <div className="text-xs space-y-0.5 mt-1">
                <p className="text-gray-600"><span className="font-semibold">Part #:</span> {tx.item_part_number || "N/A"}</p>
                <p className="text-gray-600 truncate"><span className="font-semibold">Desc:</span> {tx.item_description || "N/A"}</p>
                <p className="text-gray-600"><span className="font-semibold">Mfr:</span> {tx.item_manufacturer || "N/A"}</p>
                <p className="text-gray-600"><span className="font-semibold">Fixture:</span> {tx.fixture_name || "N/A"}</p>
                <p className="text-gray-600"><span className="font-semibold">Qty Taken:</span> {tx.quantity_used}</p>
                {tx.remaining_quantity !== undefined && tx.remaining_quantity !== tx.quantity_used && (
                  <p className="text-orange-600 font-semibold">Remaining: {tx.remaining_quantity}</p>
                )}
                <p className="text-gray-500"><span className="font-semibold">Date:</span> {tx.created_at.substring(0, 10)}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border rounded-lg shadow p-3">
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="block text-xs font-semibold text-gray-700 mb-1">Return Quantity</label>
              <input
                type="number"
                min="1"
                max={tx.remaining_quantity !== undefined ? tx.remaining_quantity : tx.quantity_used}
                step="1"
                className={`w-full border p-1.5 rounded text-sm ${quantityError ? "border-red-500" : ""}`}
                value={quantity}
                onChange={handleQuantityChange}
                onBlur={handleQuantityBlur}
                placeholder={`Max ${tx.remaining_quantity !== undefined ? tx.remaining_quantity : tx.quantity_used}`}
              />
              {quantityError ? (
                <p className="text-red-500 text-[10px] mt-0.5">{quantityError}</p>
              ) : (
                <p className="text-gray-500 text-[10px] mt-0.5">Max: {tx.quantity_used}</p>
              )}
            </div>
