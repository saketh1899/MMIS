...
export default function LowStockReportPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* DOWNLOAD BUTTON */}
      <div className="max-w-6xl mx-auto px-8 mb-6 flex justify-end">
        <button
          onClick={downloadCSV}
                ...
                <tbody>
                  {lowStockItems.map((item) => (
                    ...
                    >
                      ...
                      <td className={`p-3 ${selectedItemId === item.item_id ? "text-blue-700 font-bold" : ""}`}>{item.item_name}</td>
                      <td className="p-3">{item.item_part_number || "N/A"}</td>
                      <td className="p-3 max-w-xs truncate">{item.item_description || "N/A"}</td>
                      <td className="p-3 text-red-600 font-semibold">{item.item_current_quantity}</td>
